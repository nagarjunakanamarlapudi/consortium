...
## Conventions
- **Pass real content in `args`, as a real JSON object** — e.g. `{ plan: "…full text…" }`, **not** a JSON-encoded string. The runtime hands `args` to the script verbatim, so a stringified object makes `args.plan` `undefined`. The script and its agents see *only* what you put in `args` (the runtime can't see your conversation); never leave it empty or send a summary. (Scripts also defensively `JSON.parse` a string `args` as a safety net.)
